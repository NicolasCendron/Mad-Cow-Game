...
int desenha_esferas();
int testa_colisao_esfera_vaca(glm::vec4 const& posicao_esfera);
void desenha_cada_esfera(glm::mat4& modelo,glm::vec4& posicao_esfera);
...
int main(int argc, char* argv[])
{
    ...
    //Impede que sejam colocadas duas esferas na mesma posição X e mesma faixa
    ...
    {
        esferas.emplace_back(posicao.first, 0.0f, posicao.second, 0);
    }
    //Ordena os valores X
//    std::sort(esferas.begin(), esferas.end(), [](glm::vec4 const& lhs, glm::vec4 const& rhs) {return lhs.x < rhs.x;});
...
//    }
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "Vaca Devoradora de Mundos", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
int desenha_esferas()
{
    ...
    // Se não foi coletado ainda.

    for (auto& esfera : esferas)
    {
        if(esfera.w == 0)
        {
            desenha_cada_esfera(model,esfera);
        }
        colidiu = testa_colisao_esfera_vaca(esfera);
        if(colidiu == 1)
        {
            esfera.w = acrescenta_pontos = 1;
        }
    }
    return acrescenta_pontos;
}

void desenha_cada_esfera(glm::mat4& model,glm::vec4& posicao_esfera)
{
...
int testa_colisao_esfera_vaca(glm::vec4 const& posicao_esfera)
{
    if(posicao_esfera.w == 1)
    {
        return 0;
    }
    float raio_esfera = 0.05f;
